...
| `festina doctor --fix` | Same report, then actually installs whatever's missing (required and optional both) via the detected package manager — `apt` on Linux, Homebrew on macOS, MSYS2's `pacman` on Windows — instead of leaving the printed hint for a human to copy by hand. Prints the exact command first and asks for confirmation (`--yes`/`-y` skips that); refuses to guess for any other package manager or run non-interactively without `--yes`, rather than either doing nothing or running an install nobody agreed to. |
