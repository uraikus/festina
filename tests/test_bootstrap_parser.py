"""bootstrap/parser.f -- Festina's parser, written in Festina (claude.md #273).

A PARTIAL port, checked the same way the lexer's full one is: both
parsers emit one canonical AST dump and they must agree, node for node,
over every .f file in the repository.

The partial-ness is measured rather than assumed. A file whose Festina
dump contains an `(UNPORTED ...)` node is reported as unported, never as
a match -- so the coverage number can only move when a construct is
really implemented, and a construct that silently mis-parsed instead
would show up as a difference, not as progress.
"""
import os
import sys
