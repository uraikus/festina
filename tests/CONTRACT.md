...
All 880 tests in this directory pass against it: 589 need no external
tool at all (parser/semantic/IR-level tests, no compile-and-run step),
276 more need a working C compiler, plus 2 more
(`tests/test_packaging.py`) given `pyinstaller` too, plus 13 more given
`Xvfb`+`xdotool` too (4 of those also need `xwd`, from the same
x11-apps/x11-utils tier, to read real canvas pixels back --
claude.md #89/#92) (589 + 276 + 2 + 13 = 880 -- re-verified directly
...
pytest tests/                          # 589 passed, 291 skipped (needs a C compiler; 2 of
                                        # those skips need `pip install pyinstaller` too,
                                        # 13 need Xvfb + xdotool installed too, 1 of those
