...
- **`bootstrap/escape.f`** — `festina/escape_analysis.py` ported to
  ...
  that was not in its original estimate: **97 of 115 corpus files
  produce an identical record sequence, 0 differ, 7 not yet ported, 11
  rejected by both** — 1,800 of 1,853 records. It decides whether every
...
- **`bootstrap/codegen.f`** — `festina/codegen.py` ported to Festina,
  the fourth and last stage: **50 of 115 corpus files emit
  byte-identical LLVM IR, 0 differ, 54 not yet ported, 11 rejected by
  both** — 269,178 of 294,353 file-specific IR lines. **The bootstrap
...
- **`bootstrap/canary.py` — the breakages the harness is supposed to
  ...
  not. Eighty-one deliberate breakages, one per mechanism, re-run by
