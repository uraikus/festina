...
- **The bootstrap compiler is complete.** `bootstrap/` holds Festina's
  ...
  Nothing in the shipped compiler depends on this directory; it exists
  to be a demanding real program in the language and to be checked
...
- **`bootstrap/escape.f`** — `festina/escape_analysis.py` ported to
  ...
  that was not in its original estimate: **98 of 116 corpus files
  produce an identical record sequence, 0 differ, 7 not yet ported, 11
  rejected by both** — 1,833 of 1,886 records. It decides whether every
...
- **`bootstrap/codegen.f`** — `festina/codegen.py` ported to Festina,
  ...
  merely getting the same answers. And the `color`, `img` and `aud`
  types — a colour packed into one register with the 148-name CSS table
  ...
  cleanup stack so a throw can unwind frames that never heard of it. And
  the first slice of THREADS — `thread NAME { ... }` compiling to a
  registered handle plus three real adapter functions (emitted whether
  or not the matching handler was declared, since the C runtime reads
  all three unconditionally), thread-private state as namespaced
  globals whose initializers run inside `on load` on that thread's own
  OS thread, and both directions of `postMessage`: the bare
  context-implicit send to main, and `NAME.postMessage(x)` naming the
