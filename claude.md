...
Windows: 21 of 26 failures traced to one root cause. `mingw-w64-ucrt-x86_64-libgnurx` -- windows.md's own preferred, documented answer for the one core-runtime gap -- IS a real, installable MSYS2 package, but `pacman --noconfirm` silently drops it from the install set because it CONFLICTS with `mingw-w64-ucrt-x86_64-libsystre`, already pulled in transitively by something else in the UCRT64 toolchain. No error, no warning surfaced as a failure -- just a `pkg-config --cflags libgnurx` that came up empty two steps later, in a completely different part of the run. libsystre is not the divergent-ERE fallback windows.md reserved (vendoring musl's regex engine) -- it's a real, already-present POSIX regex.h provider, so `_core_pkgs` now asks pkg-config for it directly instead. The remaining 5 Windows failures were independent, genuine bugs of their own: `tests/conftest.py`'s `write_text`/`subprocess.run` calls used Python's locale-default encoding instead of UTF-8 on both the write and read side, which is harmless everywhere the default happens to BE UTF-8 (Linux, macOS) but corrupted a non-ASCII literal on Windows and then crashed decoding the compiled program's real UTF-8 output under the wrong codec -- now explicit both ways, matching festina/imports.py's own already-explicit UTF-8 file reads. And one test asserted `str(CompileError).split(":")[0].endswith(...)` to isolate the file path, which breaks the instant the path itself contains a colon -- a Windows drive letter, always -- now `rsplit(":", 4)`, peeling off exactly the four format-string colons (line/column/"error:"/message) from the right regardless of what the path itself contains.

Nothing here needed a design change -- every fix is a one-line correction to something written without the real environment to test it against, which is the entire reason macos.md and windows.md both budgeted for exactly this kind of round from the start. Verified: the macOS fix compiles clean with the real cairo-xlib pkg-config flags on Linux (the include is shared code); the Windows fixes are pure Python, unit-tested and re-run through the full suite (1296 passed, 7 skipped, unchanged) plus scripts/leak_stress.sh, same as every other round -- the actual libsystre pkg-config behavior remains unconfirmed until the next real Windows run, exactly the honesty windows.md's status block already commits to.
