...
Coverage is the set of sites where a text temporary is both produced and discarded: `log()`, user function calls, and the text/regex methods (`.replace()`, `.replaceAll()`, `.match()`, `.test()`). Text arguments to the graphics, sqlite, and timer builtins are not yet freed; those leak a bounded amount per call and are tracked in todo.md rather than silently claimed as done.
