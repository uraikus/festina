...
Nothing here needed a design change -- every fix is a one-line correction to something written without the real environment to test it against, which is the entire reason macos.md and windows.md both budgeted for exactly this kind of round from the start, including the round where the FIX needed its own fix. Verified: the macOS include fix compiles clean with the real cairo-xlib pkg-config flags on Linux (the include is shared code); the Windows fixes are pure Python, unit-tested and re-run through the full suite under both this sandbox's Python 3.11 and a real 3.12.3 venv (1296 passed, 7 skipped either way) plus scripts/leak_stress.sh. What remains unconfirmed is the one thing that can only be confirmed by actually running on Windows/MSYS2 again -- whether `gnurx.pc`'s cflags/libs are themselves correct and sufficient -- exactly the honesty windows.md's status block already commits to.
