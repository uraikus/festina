...
A local struct variable that is proven to escape its declaring function or handler is also reference counted, and its own reference is released when its declaring scope ends -- the same scope-exit points sections 74 through 76 already track (a function or handler's own end, a block's end, every loop iteration, break, continue). Unlike the starting scope this section originally shipped with, a local's declaration including an initializer, and a local ever being the target of a plain reassignment, no longer exclude it: both cases now retain the new value before releasing the old one, exactly as a global's own reassignment already did, so two different bindings that turn out to reference the same value no longer risk a double release. The retain is skipped only when the source of the new value is a plain function call (`Point r = make(n)`, `p = make(n)`) -- a freshly returned value nothing else yet references, whose own +1 transfers cleanly into the new binding -- and is applied for every other shape a source expression can take (reading an existing identifier, a field, a ternary, ...), since any of those might already alias a value some other tracked binding references. The one condition that still excludes a local from this section's coverage is being returned anywhere in the function: a function's own Return statement does not yet retain the value it hands back to its caller, so a local that is ever returned continues to leak exactly as before this section existed, to avoid releasing a value out from under a caller who is about to start relying on it. In the common case where a caller immediately captures a call's result in its own local (`Point r = make(n)`), that caller-side local is itself now tracked correctly by the retain/release logic this paragraph describes -- so the value is still freed once the caller's own copy goes out of scope, even though the callee's Return never retained it on the way out. Only a returned value that is discarded outright, never bound to anything (a bare `make(n)` used as a statement on its own), still leaks unconditionally. Retaining every value a function returns is expected to close this last gap in a later stage.

Struct fields, and arr[T]/map[T] values generally, are not covered by this section -- see section 76's own boundary and todo.md for why extending reference counting to a struct's own fields needs real aliasing analysis, not a small extension of this section's rule.
