...
A local struct variable that is proven to escape its declaring function or handler is also reference counted, and its own reference is released when its declaring scope ends -- the same scope-exit points sections 74 through 76 already track (a function or handler's own end, a block's end, every loop iteration, break, continue) -- but only when all three of the following hold: it was declared without an initializer, it is never itself returned anywhere in the function, and it is never itself the target of a plain reassignment anywhere in the function. A local that fails any of these conditions continues to leak exactly as before this section existed. This is a deliberately conservative starting scope, not the complete answer sections 74-76 themselves eventually reached for non-escaping locals: a value declared with an initializer, or ever reassigned, may alias a value some other tracked binding also references, and this section does not yet retain a value when a plain local variable (as opposed to a global) starts referencing it -- without that retain, releasing two different bindings that turned out to reference the same value would release it twice. Retaining on every local assignment, and every value returned from a function, is expected to widen this coverage in a later stage; extending it now, without that retaining half, would risk freeing a value another binding still needs, which section 54's ambiguity rule treats as unacceptable regardless of how much less memory would go unreclaimed by waiting.
